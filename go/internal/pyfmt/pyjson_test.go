...
// ~/.agentfield/packages/cloudsecurity-af/venv/bin/python (CPython 3.11.12); the
...
func TestDumpsContainerSeparators(t *testing.T) {
	value := Ordered{{K: "a", V: 1}, {K: "b", V: []any{1, 2}}}
...
func TestDumpsEmptyContainers(t *testing.T) {
	value := Ordered{{K: "a", V: []any{}}, {K: "b", V: Ordered{}}}
	want := "{\n  \"a\": [],\n  \"b\": {}\n}"
	if got := Dumps(value, 2); got != want {
		t.Fatalf("got\n%s\nwant\n%s", got, want)
	}
...
func TestDumpsMapKeysAreSorted(t *testing.T) {
	...
	o := Ordered{{K: "zebra", V: 1}, {K: "Apple", V: 2}}
...
// ptrMarshaler exercises the pointer-receiver marshaler path. It has no state:
// MarshalJSON returns a constant, which is the point — the test asserts that
// the encoder finds and re-renders a POINTER receiver's marshaler.
...
func TestDumpsHonorsJSONMarshaler(t *testing.T) {
	value := Ordered{
		{K: "ts", V: marshalerStamp{text: "2026-01-02T03:04:05.123456+00:00"}},
		{K: "raw", V: json.RawMessage(`{"b":2,"a":1}`)},
	}
...
// TestDumpsBytesAreBase64 documents the []byte rendering. No CloudSecurity AF model has a
