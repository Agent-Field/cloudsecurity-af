...
//
// This file is shared VERBATIM with the sec-af Go port
// (sec-af/go/internal/pyfmt/pyjson.go). The only permitted deltas are the two
// marked below:
//
//  1. this call-site list, which names cloudsecurity-af's own json.dumps sites;
//  2. `p.K` / `p.V` in pairs(), because this repo's pyfmt.KV spells its fields
//     K and V where sec-af's spells them Key and Value. Everything else —
//     encoder, escaping, float rendering, struct walk — is byte-identical.
//
// CloudSecurity AF embeds json.dumps output directly in prompt text, in
// checkpoint files and in output artifacts:
//
//	src/cloudsecurity_af/output/sarif.py:59              json.dumps(sarif, indent=2)
//	src/cloudsecurity_af/output/json_output.py:17,79     json.dumps(..., indent=2)
//	src/cloudsecurity_af/orchestrator.py:223             json.dumps(body, indent=2)
//	src/cloudsecurity_af/agents/recon/_terraform_parser.py:277  json.dump(inventory, f, indent=2, default=str)
//	src/cloudsecurity_af/agents/chain/path_constructor.py:108   json.dumps(compact_findings, indent=2)
//	src/cloudsecurity_af/agents/prove/static_prover.py:47       json.dumps(finding.model_dump(), indent=2)
//
...
// zero-width indentation, a spelling no CloudSecurity AF call site uses (every
// call is either json.dumps(x) or json.dumps(x, indent=2)).
//
// Value mapping:
//
...
//     bytes must match Python exactly — e.g. a Terraform resource's `config`
//     dict, whose insertion order is the order the attributes appear in the
//     .tf file — must build a pyfmt.Ordered instead of a map. Structs need no
//     such care: Go declaration order is pydantic declaration order, which is
//     model_dump()'s insertion order.
...
func (e *jsonEncoder) sequence(rv reflect.Value, depth int) {
	// []byte is base64 in encoding/json. No pydantic model in CloudSecurity AF
	// has a bytes field, so this branch exists only so a stray []byte cannot
...
func (e *jsonEncoder) pairs(o Ordered, depth int) {
	...
	for i, p := range o {
		...
		e.writeString(p.K)
		e.b.WriteString(": ")
		e.value(reflect.ValueOf(p.V), depth+1)
	}
	e.newline(depth)
	e.b.WriteByte('}')
}
...
// IsEmptyValue is encoding/json's omitempty predicate, exported so the pydantic
// serializer in internal/output can share it instead of keeping a second copy.
...
//
// Exported so internal/output's pydantic serializer shares it — the coercion is
// encoding/json's, identical on both sides, and was duplicated verbatim.
