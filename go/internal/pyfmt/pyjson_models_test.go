...
//
// Every golden here is produced by go/scripts/gen_golden_output.py from the REAL
// pydantic model:
//
...
func loadGolden(t *testing.T, name string) string {
	...
	if err != nil {
		t.Fatalf("read golden %s: %v (regenerate with go/scripts/gen_golden_output.py)", name, err)
	}
	return string(raw)
}
...
// for four real CloudSecurity pydantic models built from one fixture, the Go
// rendering of the Go struct equals CPython's json.dumps of the model_dump()
// dict, in both the indent=2 and the compact spelling.
//
// The chosen models between them cover every shape the port has to render:
// nested lists of models (AttackPath.steps, VerifiedFinding.resources), a nested
// sub-model (Proof, BlastRadius), nullable models and scalars left at None
// (attack_path, drift, remediation, drop_reason, estimated_data_volume), empty
// lists, a str-Enum field (verdict, severity, method, combined_severity), a
// dict[str, float] (ScanMetrics.cost_breakdown), a bool, and the awkward float
// spellings (1e-05, 1e+16, 1000000000000000.0, -0.0, 10.0) plus unicode and
// control characters that ensure_ascii must escape.
func TestDumpsMatchesPythonModelDump(t *testing.T) {
	...
	cases := []struct {
		...
		{"VerifiedFinding", decodeInto[schemas.VerifiedFinding]},
		{"AttackPath", decodeInto[schemas.AttackPath]},
		{"ChainResult", decodeInto[schemas.ChainResult]},
		{"ScanMetrics", decodeInto[schemas.ScanMetrics]},
	}
...
// stay integers, which is what Python's json.loads does.
//
...
// makes a 200-line model dump diagnosable without printing both in full.
