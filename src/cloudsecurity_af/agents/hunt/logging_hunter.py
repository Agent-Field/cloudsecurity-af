...
PROMPT_PATH = Path(__file__).resolve().parents[4] / "prompts" / "hunt" / "logging.txt"
...
async def run_logging_hunter(
    ...
    resource_graph_summary, inventory_stats, relevant_edges = build_graph_context_for_hunter(
        resource_graph_path,
        inventory_path,
        ["cloudtrail", "flow_log", "guardduty", "cloudwatch", "log_group", "access_log", "waf_log"],
    )
