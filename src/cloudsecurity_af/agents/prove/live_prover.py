...
PROMPT_PATH = Path(__file__).resolve().parents[4] / "prompts" / "prove" / "live_prover.txt"
...
async def run_live_prover(
    ...
    harness_cwd = tempfile.mkdtemp(prefix="cloudsecurity-live-prover-")
    try:
        ...
        return extract_harness_result(result, VerifiedFinding, "LiveProver")
    finally:
